...
requiredData = data((data(:,9) >= 0 & data(:,9) <= 3) | data(:,9) == 6,2:5);
...
for i = 1:num_starts
    current_net = feedforwardnet(5);
    
    current_net.trainFcn = 'trainbr'; 
    current_net.trainParam.epochs = 100; % Set the maximum number of epochs
    current_net.trainParam.goal = 1e-3; % Set the performance goal
    current_net.trainParam.showWindow = true; % Show training progress window
    current_net.trainParam.min_grad = 1e-2;
    current_net.trainParam.max_fail = 6;
    current_net.divideParam.trainRatio = 0.8;
    current_net.divideParam.valRatio   = 0.2;
    current_net.divideParam.testRatio  = 0;
    current_net.layers{1}.transferFcn = 'logsig';
    current_net.layers{2}.transferFcn = 'logsig';

    [current_net, current_tr] = train(current_net, train_inputs', train_targets');
    
    current_performance = perform(current_net, train_targets', current_net(train_inputs'));
